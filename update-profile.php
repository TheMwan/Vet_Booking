...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($check_username_result->num_rows > 0) {
            ...
            echo "<script>alert('Username already exists. Please choose a different username.');window.location.href = 'user.html';</script>";
        } else {
            ...
            if ($check_email_result->num_rows > 0) {
                ...
                echo "<script>alert('Email already exists. Please choose a different email.');window.location.href = 'user.html';</script>";
            } else {
                ...
                if ($username == $user['UserName'] && $name == $user['Name'] && $email == $user['Email'] && $phone == $user['Phone']) {
                    ...
                    if ($update_stmt->execute()) {
                        // If the update is successful, redirect to user.html to reflect changes
                        header("Location: user.html");
                        exit();
                    } else {
                        // Error updating the profile
                        echo "<script>alert('Error updating profile: " . $update_stmt->error . "');window.location.href = 'user.html';</script>";
                    }
                }
            }
        }
